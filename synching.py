...
from typing import List, Any, Union, Callable, Dict, Iterable
...
def _parse_rclone_progress(actions:List[AbstractSyncAction], sync_direction: ActionDirection, output:Dict) -> SyncProgress:
    '''
...
class AbstractSyncAction(ABC):
    ...
    def _validate_action_direction(this):
        if this.direction is not None:
            if (this.direction == ActionDirection.SRC2DST) and (not this.a.exists):
                raise SyncDirectionNotPermittedException("Destination file does not exist to support a source-to-destination operation")
            elif (this.direction == ActionDirection.DST2SRC) and (not this.b.exists):
                raise SyncDirectionNotPermittedException("Source file does not exist to support a destination-to-source operation")
    ...
    def swap_direction(this) -> None:
        ...
        if not fs.exists:
            raise SyncDirectionNotPermittedException(f"Action swapping not possible because {fs.absolute_path} does not exist")
...
class SynchingManager():
    ...
    def replace(this, action: AbstractSyncAction, replace_with: AbstractSyncAction) -> None:
        idx = this._changes.index(action)
        this._changes[idx] = replace_with
